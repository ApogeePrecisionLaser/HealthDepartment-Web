...
ShiftWorkBench-2
 */
package com.healthDepartment.shift.model;
...
//        searchemp = krutiToUnicode.convert_to_unicode(searchemp);
    ...
    public List<ShiftLoginBean> showDataBean(String emp_code) {
        ...
        String query = "SELECT * FROM (SELECT cordinate_id, latitude, longitude, imei_no "
    ...
    public JSONObject getAreaDetails(String emp_code) {
        ...
        String query = "  select lt.location_type_name,z.zone_name,w.ward_name,a.area_name,cl.location, "
                + "  kp.key_person_id,kp.key_person_name,kp.emp_code,kp.mobile_no1,st.shift_type,d.designation, "
    ...
    public int updateAttendence(String skpm_id) {
      ...
      String start_time= dateFormat.format(date);
        String query = "update attendence set attendence='Y',start_time='"+start_time+"' where shift_key_person_map_id=? ";
        try {
            PreparedStatement ps = (PreparedStatement) connection.prepareStatement(query);
    ...
    public int updateFinishTime(String skpm_id) {
        int rowsAffected = 0;
        DateFormat dateFormat = new SimpleDateFormat("HH:mm");
      Date date = new Date();
      String end_time= dateFormat.format(date);
        String query = "update attendence set end_time='"+end_time+"' where shift_key_person_map_id=? ";
        try {
            PreparedStatement ps = (PreparedStatement) connection.prepareStatement(query);
            ps.setInt(1, Integer.parseInt(skpm_id));
            rowsAffected = ps.executeUpdate();
    ...
    public void sendMessageToNextEightBeneficiary() {
    }
    ...
    public JSONArray getVehicleKeyPersonDetails(String emp_code) {
        JSONArray jsonArray = new JSONArray();
        String query = " select v.vehicle_code,v.vehicle_no "
                + " from vehicle_key_person vkp,vehicle v,key_person as kp,shift_key_person_map as skpm "
                + " where vkp.vehicle_id=v.vehicle_id and vkp.shift_key_person_map_id=skpm.shift_key_person_map_id and skpm.key_person_id=kp.key_person_id"
                + " and kp.emp_code=" + emp_code;
        try {
            ResultSet rs = connection.prepareStatement(query).executeQuery();
            while (rs.next()) {
                JSONObject jsonObj = new JSONObject();
                jsonObj.put("vehicle_code", rs.getInt("vehicle_code"));
                jsonObj.put("vehicle_no", rs.getString("vehicle_no"));
                jsonArray.put(jsonObj);
            }
        } catch (Exception ex) {
            System.out.println("ERROR : in getVehicleKeyPersonDetails  in ShiftLoginModel : " + ex);
        }
        return jsonArray;
    }

    public JSONArray getvehicleDetails(String zone, String ward, String area) {
        JSONArray jsonArray = new JSONArray();
        String query = " Select vehicle_code,vehicle_no,kp.emp_code from shift_key_person_map as skpm"
                ...
                + "     left join vehicle_key_person as vkp on vkp.shift_key_person_map_id=skpm.shift_key_person_map_id and vkp.active='Y'"
                + "       left join vehicle as v on v.vehicle_id=vkp.vehicle_id  where skpm.active='y' and lt.location_type_name='Ward' and d.designation='ड्राइवर' "
                + "        group by vkp.shift_key_person_map_id ";
        try {
            ResultSet rs = connection.prepareStatement(query).executeQuery();   
            while (rs.next()) {
                JSONObject jsonObj = new JSONObject();
                jsonObj.put("vehicle_no", rs.getInt("vehicle_no"));
                jsonObj.put("vehicle_code", rs.getInt("vehicle_code"));
                jsonObj.put("emp_code", rs.getInt("emp_code"));
    ...
    public int insertVehicle(String emp_code_givenBy, String emp_code_takenBy, String latitude, String longitude, String date_time, String status) {
        String query = "";
        int rowsAffected = 0;
        ...
        try {
            ...
            String shift_key_person_map_id2 = getSkpmIdFromVehicleCode(emp_code_takenBy);
            ps.setInt(2, Integer.parseInt(shift_key_person_map_id2));
            int vehicle_id = getVehicleId(emp_code_givenBy);
    ...
    public int getVehicleId(String vehicle_code) {
        int vehicle_id = 0;
        String query = "select v.vehicle_id from vehicle_key_person vkp,key_person kp,vehicle v,shift_key_person_map skpm "
                + " where skpm.key_person_id=kp.key_person_id and vkp.shift_key_person_map_id=skpm.shift_key_person_map_id "
                + " and vkp.vehicle_id=v.vehicle_id and emp_code=" + vehicle_code;
        try {
            PreparedStatement pst = connection.prepareStatement(query);
            ResultSet rs = pst.executeQuery();
            while (rs.next()) {
    ...
    public String getSkpmIdFromVehicleCode(String vehicle_code) {
        String vehicle_id = "";
        String query = " select vkp.shift_key_person_map_id from vehicle_key_person vkp,key_person kp,vehicle v,shift_key_person_map skpm "
                + " where skpm.key_person_id=kp.key_person_id and vkp.shift_key_person_map_id=skpm.shift_key_person_map_id "
                + " and vkp.vehicle_id=v.vehicle_id and vehicle_code=" + vehicle_code;
        try {
            PreparedStatement pst = connection.prepareStatement(query);
            ResultSet rs = pst.executeQuery();
            while (rs.next()) {
                vehicle_id = rs.getString("shift_key_person_map_id");
            }
        } catch (Exception e) {
            System.out.println(e);
        }
        ...
        public int updateVehicleVerify(String skpm_id) {
        int rowsAffected = 0;
        String query = "update vehicle_key_person set verify='Y' where shift_key_person_map_id=? ";
        try {
            PreparedStatement ps = (PreparedStatement) connection.prepareStatement(query);
            ps.setInt(1, Integer.parseInt(skpm_id));
            rowsAffected = ps.executeUpdate();
   ...
   public int InsertErrorLog(String vehicle_code) {
        String query = "";
        int rowsAffected = 0;
        query = "insert into error_log(error_causeby_id,error_sendto_id,type_of_error_id,status_id) "
                + "values(?,?,?,?)";

        try {
            PreparedStatement ps = (PreparedStatement) connection.prepareStatement(query);
            String shift_key_person_map_id1 = getSkpmIdFromVehicleCode(vehicle_code);
            ps.setInt(1, Integer.parseInt(shift_key_person_map_id1));
            ps.setInt(2, 44);
            ps.setInt(3, 2);
            ps.setInt(4, 1);
