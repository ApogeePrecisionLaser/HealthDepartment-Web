...
import com.healthDepartment.shift.tableClasses.ShiftKeyPersonMapBean;
...
//         searchperson = krutiToUnicode.convert_to_unicode(searchperson);
         ...
         String addQuery = " LIMIT " + lowerLimit + ", " + noOfRowsToDisplay;
          ...
          if(location_type.equals("ward"))
            ...
            String query= "Select skpm.shift_key_person_map_id,st.shift_type,d.designation,lt.location_type_name,z.zone_name,"
                             + " z.zone_no,w.ward_name,w.ward_no,a.area_name,a.area_no,cl.location,cl.location_no,kp.key_person_name,kp.emp_code,"
                             + " skpm.date,skpm.remark,vehicle_code "
                             + " from shift_key_person_map as skpm "
                             ...
                             + " left join vehicle as v on v.vehicle_id=vkp.vehicle_id "
                             + " where skpm.active='y' " + addloc
                             + " AND IF('"+ search_shift_type +"'='', st.shift_type LIKE '%%', st.shift_type='"+ search_shift_type +"')"
                             + " AND IF('"+ search_designation +"'='', d.designation LIKE '%%', d.designation='"+ search_designation +"')"
                             + " AND IF('"+ searchperson +"'='', kp.key_person_name LIKE '%%', kp.key_person_name='"+ searchperson +"')"
       ...
       PreparedStatement ps = (PreparedStatement) connection.prepareStatement(query);
         ...
         while(rs.next()){
             ...
             String date=""+rs.getDate("date");
              ...
              skpm.setVehicle_code(rs.getString("vehicle_code"));
              list.add(skpm);
          }
          }
            catch(Exception e)
     ...
     public boolean insertRecord(ShiftKeyPersonMapBean bean){
        ...
        int skpm=0;
         try{
             ...
             if(rowsAffected > 0)
                 insertRecord(skpm,bean.getVehicle_code());
                 status=true;
             }
 ...
 public boolean insertRecord(int skpm,String vehicle_code) {
        boolean status = false;
        String query = "";
        int rowsAffected = 0;
        ...
        try {
            ...
            ps.setInt(1, skpm);
            ps.setInt(2,Integer.parseInt(vehicle_code));
