 ...
 * @author Shobha
 */
public class VehicleKeyPersonPointMISModel {
private static Connection connection;
 ...
 public static int  getNoOfRows(String search_point_name,String search_key_person_name)
 {
      ...
      search_key_person_name = krutiToUnicode.convert_to_unicode(search_key_person_name);
        ...
        //String query="SELECT count(vehicle_key_person_point_id) from vehicle_key_person_point ";
            ...
            String query="SELECT  count(*) "
                    +" from vehicle_key_person_point vkpp,vehicle_key_person vkp,key_person kp,point p "
                    +" where vkpp.vehicle_key_person_id=vkp.vehicle_key_person_id "
                    +" and vkp.key_person_id=kp.key_person_id "
                     +" and vkpp.point_id=p.point_id "
                     + "AND vkpp.active='Y' "
                     + "AND vkp.active='Y' "
 ...
 /////////////////////////////////////////////////////////////////////////////////////////////////////////////
 public List<VehicleKeyPersonPointMIS> showDataBean(String search_key_person_name,String search_point_name) {
        List<VehicleKeyPersonPointMIS> list = new ArrayList<VehicleKeyPersonPointMIS>();
       ...
       String query="SELECT  p.point_name,p.latitude,p.longitude "
        ...
        try {
            ...
            ResultSet rset = pstmt.executeQuery();
            while (rset.next()) {
                VehicleKeyPersonPointMIS bean = new VehicleKeyPersonPointMIS();
 ...
 public static List<VehicleKeyPersonPointMIS> showData(int lowerLimit,int noOfRowsToDisplay,String search_point_name,String search_key_person_name)
  {
        search_point_name = krutiToUnicode.convert_to_unicode(search_point_name);
        search_key_person_name = krutiToUnicode.convert_to_unicode(search_key_person_name);
       List list = new ArrayList();
         ...
         String addQuery = " LIMIT " + lowerLimit + ", " + noOfRowsToDisplay;
          if(lowerLimit == -1)
            addQuery = "";
            String query="SELECT  vkpp.vehicle_key_person_point_id,kp.key_person_name,p.point_name "
                    +" from vehicle_key_person_point vkpp,vehicle_key_person vkp,key_person kp,point p "
                    +" where vkpp.vehicle_key_person_id=vkp.vehicle_key_person_id "
                    +" and vkp.key_person_id=kp.key_person_id "
                     +" and vkpp.point_id=p.point_id "
...
public  boolean insertRecord(VehicleKeyPersonPointMIS bean)
 {
...
int rowsAffected=0;
      ...
      if(vehicle_key_person_point_id==0)
         query="insert into vehicle_key_person_point(vehicle_key_person_id,point_id) values(?,?)";
//      if(vehicle_key_person_point_id>0)
...
public boolean reviseRecords(VehicleKeyPersonPointMIS bean){
    ...
    int rowsAffected=0;
     ...
     int rev_no=0;
      ...
      String query3 = "INSERT INTO vehicle_key_person_point (vehicle_key_person_point_id,vehicle_key_person_id,point_id, rev_no, active) VALUES (?,?,?,?,?) ";
...
public static List<String> getKeyPerson(String q)    {
     List<String> list=new ArrayList<String>();
 ...
 public int getVehicleKeyPersonId(String key_person){
     ...
     String query="select vehicle_key_person_id from key_person kp,vehicle_key_person vkp "
             +" where key_person_name= '"+key_person+"'"
             +" and kp.key_person_id=vkp.key_person_id";
     try{
     PreparedStatement pst = connection.prepareStatement(query);
     ResultSet rs=pst.executeQuery();
     while(rs.next()){
