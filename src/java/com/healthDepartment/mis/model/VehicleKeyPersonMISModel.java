...
import com.healthDepartment.mis.tableClasses.VehicleKeyPersonMIS;
 ...
 * @author Shobha
 */
public class VehicleKeyPersonMISModel {
private static Connection connection;
 ...
 public static int  getNoOfRows(String vehicleType,String key_person_name)
 {
      ...
      key_person_name = krutiToUnicode.convert_to_unicode(key_person_name);
        int noOfRows = 0;
        try {
        String query="SELECT count(*) from vehicle_key_person vkp,vehicle v,key_person kp "
 ...
 public static List<VehicleKeyPersonMIS> showData(int lowerLimit,int noOfRowsToDisplay,String vehicleType,String key_person_name)
  {
        //vehicleType = krutiToUnicode.convert_to_unicode(vehicleType);
     vehicleType = krutiToUnicode.convert_to_unicode(vehicleType);
      key_person_name = krutiToUnicode.convert_to_unicode(key_person_name);
       List list = new ArrayList();
         ...
         String addQuery = " LIMIT " + lowerLimit + ", " + noOfRowsToDisplay;
          if(lowerLimit == -1)
            addQuery = "";
            String query="SELECT  vehicle_key_person_id,key_person_name,vehicle_code "
                    + " from vehicle v,key_person kp,vehicle_key_person vkp "
                    + " where  vkp.vehicle_id=v.vehicle_id "
                       +" AND vkp.key_person_id=kp.key_person_id "
                       + "AND vkp.active='Y'"
                       + " AND IF('" + vehicleType + "' = '', vehicle_code LIKE '%%',vehicle_code='" + vehicleType + "') "
...
//                +addQuery;
    try{
       PreparedStatement ps = (PreparedStatement) connection.prepareStatement(query);
             ...
             while(rs.next()){
             VehicleKeyPersonMIS vt=new VehicleKeyPersonMIS();
...
public  boolean insertRecord(VehicleKeyPersonMIS bean)
 {
...
int rowsAffected=0;
      ...
      if(vehicle_key_person_id==0)
         query="insert into vehicle_key_person(vehicle_id,key_person_id) values(?,?)";
      if(vehicle_key_person_id>0)
...
public boolean reviseRecords(VehicleKeyPersonMIS bean){
    ...
    int rowsAffected=0;
      ...
      String query3 = "INSERT INTO vehicle_key_person (vehicle_key_person_id,vehicle_id,key_person_id, rev_no, active) VALUES (?,?,?,?,?) ";
...
public static List<String> getSearchVehicleCode(String q)    {
 ...
 public int getKeyPersonId(String key_person){
     ...
     int key_person_id=0;
     String query="select key_person_id from key_person where key_person_name= '"+key_person+"'";
     try{
     PreparedStatement pst = connection.prepareStatement(query);
     ResultSet rs=pst.executeQuery();
     while(rs.next()){
